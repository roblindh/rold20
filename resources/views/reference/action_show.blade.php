    ...
    <div class="border-b border-slate-200 pb-4 flex items-center justify-between">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-3xl font-bold text-slate-900">{{ $action->Name }}</h1>
            ...
            <p class="text-slate-500 text-sm mt-1">Cost: {{ $action->ActionTime ?? '1 AP' }} | Check: {{ $action->ActionCheck ?? 'None' }}</p>
        </div>
        <a href="{{ route('reference.actions') }}" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-1">
