...
if (!isset($_POST['Reset']))
{
	...
	if (isset($_POST['Str'])) $abilities->Scores[A_STR] = (int)$_POST['Str'];
	if (isset($_POST['Con'])) $abilities->Scores[A_CON] = (int)$_POST['Con'];
	if (isset($_POST['Dex'])) $abilities->Scores[A_DEX] = (int)$_POST['Dex'];
	if (isset($_POST['Int'])) $abilities->Scores[A_INT] = (int)$_POST['Int'];
	if (isset($_POST['Wis'])) $abilities->Scores[A_WIS] = (int)$_POST['Wis'];
	if (isset($_POST['Cha'])) $abilities->Scores[A_CHA] = (int)$_POST['Cha'];

	if (isset($_POST['Race'])) $race = (int)$_POST['Race'];
	...
	if (isset($_POST['Gender'])) $gender = (int)$_POST['Gender'];

	if (isset($_POST['AgeCat'])) $agecat = (int)$_POST['AgeCat'];
	if (isset($_POST['RLMod'])) $rlmod = (int)$_POST['RLMod'];
	if (isset($_POST['SizeMod'])) $sizemod = (int)$_POST['SizeMod'];

	if (isset($_POST['Culture'])) $culture = (int)$_POST['Culture'];
	if (isset($_POST['BackgndClass'])) $backgndclass = (int)$_POST['BackgndClass'];
	...
	if (isset($_POST['SC'])) $socialclass = (int)$_POST['SC'];
	if (isset($_POST['WC'])) $wealthclass = (int)$_POST['WC'];
...
if (isset($_POST['Generate']))
{
	...
	$config = $description . " { ";
	if ($abilities->Scores[A_STR] != 10) $config .= "Str=" . $abilities->Scores[A_STR] . "; ";
	if ($abilities->Scores[A_CON] != 10) $config .= "Con=" . $abilities->Scores[A_CON] . "; ";
	if ($abilities->Scores[A_DEX] != 10) $config .= "Dex=" . $abilities->Scores[A_DEX] . "; ";
	if ($abilities->Scores[A_INT] != 10) $config .= "Int=" . $abilities->Scores[A_INT] . "; ";
	if ($abilities->Scores[A_WIS] != 10) $config .= "Wis=" . $abilities->Scores[A_WIS] . "; ";
	if ($abilities->Scores[A_CHA] != 10) $config .= "Cha=" . $abilities->Scores[A_CHA] . "; ";
	if ($agecat != 3) $config .= "AgeCat=" . $_APP['agecats'][$agecat]['Description'] . "; ";
	if ($actual_culture != $_APP['creatures'][$race]['DefaultCulture']) $config .= "Culture=" . $_APP['cultures'][$actual_culture]['Name'] . "; ";
	if ($actual_backgndclass != $_APP['cultures'][$actual_culture]['ClassConfig']) $config .= "BackgndClass=" . $_APP['classconfigs'][$actual_backgndclass]['Name'] . "; ";
	if ($rlmod != 0) $config .= "RLMod=" . $rlmod . "; ";
	if ($sizemod != 0) $config .= "SzMod=" . $sizemod . "; ";
	foreach ($templates as $template)
	{
		if ($template != 0) $config .= "Template=" . $_APP['templates'][$template]['NameInformal'] . "; ";
	}
	foreach ($classes as $idx => $class)
	{
		if ($class != 0)
	...
	if ($socialclass != 0) $config .= "SC=" . $socialclass . "; ";
	if ($wealthclass != 0) $config .= "WC=" . $wealthclass . "; ";
	$config .= $equipment;
	$config .= "}";
