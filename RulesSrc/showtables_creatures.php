...
function show_creatureinfo($id, $fullinfo) {
    ...
    echo "<td colspan=2>" . $row['Name'] . " (" . $row['NameInformal'] . ")" .
    ($row['Descriptors'] ? (" - " . $row['Descriptors']) : "") . "</td>";
    echo "</tr></thead>";
    $odd = true; {
    ...
    if ($row['ExternalImageURL']) {
        ...
        echo "<td><a href=\"" . $row['ExternalImageURL'] . "\">Image</a></td>";
        echo "</tr>";
    }
    if ($row['Personality']) {
        echo "<tr class=\"tablerow" . ($odd ? "" : "alt") . "\">";
    ...
    if ($fullinfo && $row['StatBlockConfigs']) {
        ...
        echo "<tr class=\"tablerow" . ($odd ? "" : "alt") . "\">";
        $odd = !$odd;
        echo "<td>Stat Block(s):</td>";
        echo "<td>" . str_replace("\\n", "<br/>", $entity->GetStatBlocksStr($row['ID'])) . "</td>";
        echo "</tr>";
    }
    echo "</table>";
}
...
function show_creatures($typeID) {
    ...
    <p>
        ...
        <em>aDeC, pDeC:</em> Active DeC and Passive DeC.<br />
        <em>Crit:</em> Offset from aDeC/pDeC required for an attacker to achieve a critical hit.<br />
        ...
        <em>Languages:</em> The languages most commonly spoken by the creature. Usually includes R/W.<br />
...
function show_creaturetypes() {
    ...
    ?>
    <table class="table">
        <caption align="bottom">Creature Types</caption>
...
function show_templates($suitability, $fullinfo) {
    ...
    <p>
        ...
        <em>Racial Traits:</em> Special traits and bonuses due to template.<br />
    </p>
    <?php
    while ($row = mysqli_fetch_array($result)) {
