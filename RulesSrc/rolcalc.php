...
class cExpressionParser
{
    ...
    public function SetAngleUnit($unit)
    {
        ...
        private function GetNextToken()
        {
            ...
            if ((($this->currentExpression[$this->currentPosition] >= '0') && ($this->currentExpression[$this->currentPosition] <= '9'))
                || ($this->currentExpression[$this->currentPosition] == '.'))
            {
            	for ($i = $this->currentPosition + 1;
            		($i < strlen($this->currentExpression)) &&
                    ((($this->currentExpression[$i] >= '0') && ($this->currentExpression[$i] <= '9')) ||
                     (($this->currentExpression[$i] >= 'A') && ($this->currentExpression[$i] <= 'Z')) ||
                      ($this->currentExpression[$i] == '.') || ($this->currentExpression[$i] == '_'));
                     $i++) ;
            ...
            else if (($this->currentExpression[$this->currentPosition] == '(') || ($this->currentExpression[$this->currentPosition] == ')')
                  || ($this->currentExpression[$this->currentPosition] == '[') || ($this->currentExpression[$this->currentPosition] == ']')
                  || ($this->currentExpression[$this->currentPosition] == '|'))
            {
        ...
        {
            for ($argSq = 1.0 / ($arg * $arg), $n = 0.0, $i = gammaM - 1;
                 $i >= 0;
                 $n = $n * $argSq + cExpressionParser::$gammaP1[$i--])
                ;
        ...
        static private function GammaGPos($arg)
        {
            ...
            $s = $arg - 2.0;
            for ($n = $d = 0.0, $i = cExpressionParser::$gammaN - 1;
                 $i >= 0;
                 $n = $n * $s + cExpressionParser::$gammaP2[$i], $d = $d * $s + cExpressionParser::$gammaQ2[$i--])
                ;
