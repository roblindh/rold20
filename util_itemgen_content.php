...
if (!isset($_POST['Reset']))
{
	...
	if (isset($_POST['ItemId'])) $itemid = (int)$_POST['ItemId'];
	if (isset($_POST['Material'])) $material = (int)$_POST['Material'];
...
if (isset($_POST['Generate']) && $itemid > 0)
{
	...
	if ($material != 0) $config .= "Material=" . $_APP['materials'][$material]['Name'] . ": ";
	foreach ($modsmundane as $modmundane)
	{
		if ($modmundane != 0) $config .= "Mod=" . $_APP['itemmodsmundane'][$modmundane]['Abbreviation'] . ": ";
	}
	foreach ($modsmagic as $idx => $modmagic)
	{
		if ($modmagic != 0) $config .= "Mod=" . $_APP['itemmodsmagic'][$modmagic]['Abbreviation'] .
